...
from ui.pnl_attrib_date_set import PnlAttribDateSet, EVT_ATTRIB_DATE_CHG
...
class PnlCatGamesEditor(wx.Panel):
    ...
    def reload_game_attrs(self):
        ...
        if self.__selected_game != "":
            d_game_attrs = self.__gamelist_mgr.get_game_attribs(
                True,
                self.__selected_game
            )
        ...
        for attr_name, attr_type in d_all_game_attrs.items():
            ...
            if len(self.__l_gattrs) < i + 1:
                if attr_type == datetime:
                    pnl_game_attr = PnlAttribDateSet(
                        self.__scrl_pnl_attrs,
                        attr_name,
                        attr_name
                    )

                    pnl_game_attr.Bind(EVT_ATTRIB_DATE_CHG, self.date_attr_changed)
            ...
            if attr_name in d_game_attrs.keys():
                ...
                if isinstance(l_attr[1], PnlAttribDateSet):
                    l_attr[1].set_attrib_val(d_game_attrs[attr_name])
                else:
                ...
                # Элемент интерфейса, хранящий значение
                if isinstance(l_attr[1], PnlAttribDateSet):
                    l_attr[1].set_attrib_val(None)
                elif isinstance(l_attr[3], wx.SpinCtrl) \
                    or isinstance(l_attr[3], FloatSpin):
                    l_attr[3].SetValue(0)
                else:
                    l_attr[3].SetValue("")
    ...
    def date_attr_changed(self, event):
        ...
        if self.__selected_game != "":
            if self.__ignore_chg_attrib is False:
                # Пользователь изменил атрибут игры,
                # применяем его к списку игр
                pnl_gattr_date = event.GetEventObject()
                self.__gamelist_mgr.set_game_attrib(
                    self.__selected_game,
                    pnl_gattr_date.get_attrib_name(),
                    pnl_gattr_date.get_attrib_val()
                )
